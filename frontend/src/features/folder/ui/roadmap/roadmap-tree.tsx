...
function ProgressCircle({ percent }: { percent: number }) {
  ...
  return (
    <svg width="21" height="21" viewBox="0 0 21 21" fill="none" className="shrink-0">
      <circle cx="10.5" cy="10.5" r={CIRCLE_R} stroke="var(--ege-track)" strokeWidth="3" fill="none" />
      {percent > 0 ? (
        <circle
          cx="10.5"
          cy="10.5"
...
export function CheckboxChecked({ className }: { className?: string }) {
  return (
    <div className={cn("relative h-5 w-5 shrink-0 text-white", className)}>
      <CheckboxIcon className="absolute inset-0 h-5 w-5" />
      <div className="absolute h-5 w-5 rounded-full bg-[var(--ege-accent)]" />
  ...
  if (isCreating) {
    return (
      <button type="button" onClick={onToggleGroup} className="shrink-0 cursor-pointer">
        {isAllSelected ? <CheckboxChecked className="h-5 w-5" /> : <CheckboxIcon className="h-5 w-5 text-[var(--ege-border)]" />}
      </button>
    )
  }

  return (
    <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[var(--ege-track)]">
      <div className="h-1.5 w-1.5 rounded-full bg-[var(--ege-muted)]" />
    </div>
  )
}
  ...
  if (isCreating) {
    return (
      <div className="flex cursor-pointer items-center gap-2.5 px-4 py-2.5" onClick={onToggle}>
        {isSelected ? <CheckboxChecked className="h-5 w-5 shrink-0" /> : <CheckboxIcon className="h-5 w-5 shrink-0 text-[var(--ege-border)]" />}
        <span className="flex-1 nova-text-p-medium text-[var(--ege-muted)]">{lesson.name}</span>
      </div>
    )
  }

  return (
    <div className="flex items-center justify-between gap-2 rounded-full pr-1.5 pl-4 hover:bg-[var(--ege-surface)]">
      <span className="flex-1 py-2.5 nova-text-p-medium text-[var(--ege-muted)]">
        {lesson.name}
      </span>
      <Tippy
        content="Общий уровень владения темой по урокам, объяснениям и тестам. Решай больше заданий, чтобы закрепить результат."
      >
        <div className="flex shrink-0 items-center gap-3 p-2.5">
          <span className="nova-text-label-small text-[var(--ege-text)]">
            {displayPercent}%
          </span>
          {isDone ? <CheckIcon className="text-[var(--ege-accent)]" /> : <ProgressCircle percent={displayPercent} />}
        </div>
      </Tippy>
    </div>
  );
...
const BLOCK_CLASS =
  "group/block min-w-[431px] max-w-[596px] rounded-[14px] py-2 pr-2 pl-1 transition-colors hover:bg-[var(--ege-surface)]";
...
export function RoadmapSubsectionBlock({
  ...
  return (
    <div className={cn(BLOCK_CLASS, "relative")}>
      {!isLast && (
        <div
          className="absolute w-px bg-[var(--ege-border)]"
      ...
      <div className="flex items-center gap-1">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <h3 className="nova-text-label-medium text-[var(--ege-text)]">{subsection.name}</h3>
        </div>
      </div>
      {isOpen && (
      ...
      <div className={BLOCK_CLASS}>
        <div className="flex items-center gap-1">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <h2 className="nova-text-label-medium text-[var(--ege-text)]">{section.name}</h2>
          </div>
        </div>
