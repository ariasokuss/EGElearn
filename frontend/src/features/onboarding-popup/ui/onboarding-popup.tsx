...
import { getOnboardingDismissAction } from "../model/onboarding-popup-dismiss"
...
export function OnboardingPopup() {
    ...
    const handleXClick = () => {
        const action = getOnboardingDismissAction({
            watchedVideoFully: watchedVideoFully.current,
        })
        if (action === "close") {
            closeWithReset()
            return
        ...
        >
            <div className="flex flex-col w-full max-w-200 bg-white border border-[#F4F4F5] rounded-[20px]">
                ...
                <div className="relative flex justify-center items-center w-full h-full p-1.5">
                    {!hideVideoElement && (
                        <div className="relative z-10 w-full max-h-full rounded-[14px] overflow-hidden transition-all">
                            <video
                                ref={videoRef}
